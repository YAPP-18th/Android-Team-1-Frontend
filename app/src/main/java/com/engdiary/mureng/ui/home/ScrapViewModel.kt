...
abstract class ScrapViewModel constructor(
    ...
    fun addExpressionResult(questionData: TodayExpression) {
        val results: MutableList<TodayExpression> =
            _todayExpression.value?.toMutableList() ?: mutableListOf()
    ...
    fun toggleTodayExpression(preTodayExpression: TodayExpression) {
        val results: MutableList<TodayExpression> =
            _todayExpression.value?.toMutableList() ?: return
        ...
        if (replaceIndex == -1) {
            ...
            results.add(
                replaceIndex,
                preTodayExpression.copy(scrappedByRequester = preTodayExpression.clickedScrap)
            )
            _todayExpression.value = results
        }
    }
